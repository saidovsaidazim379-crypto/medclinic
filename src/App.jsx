...
const NAV = [
  ...
  {id:'obhavo',icon:'🌤',label:'Ob-havo',sec:null},
...
// ── Xonalar ────────────────────────────────────────────────────────────────
function Xonalar(){
  const [xonalar, loading, reload] = useLoad(api.getXonalar)
  const [modal, setModal] = useState(null) // null | 'add' | xona obj (edit)
  ...
  const TURLAR = ['Umumiy','Yakka','Reanimatsiya','Operatsiya','VIP']
  const DEF = {raqam:'',tur:'Umumiy',sig:2,band:0,qavat:1}

  const jamiSig = (xonalar||[]).reduce((a,x)=>a+x.sig,0)
  const bandJami = (xonalar||[]).reduce((a,x)=>a+x.band,0)
  const fil = (xonalar||[]).filter(x=> filterTur==='Barchasi' || x.tur===filterTur)

  const save = async (data) => {
    setSaving(true)
    try {
      if(modal==='add') await api.addXona({...data, sig:+data.sig, band:+data.band, qavat:+data.qavat})
      else await api.updXona(modal.id, {...data, sig:+data.sig, band:+data.band, qavat:+data.qavat})
      await reload(); setModal(null)
    } catch(e){ alert('❌ '+e.message) }
    setSaving(false)
  }
  ...
  const del = async (x) => {
    setSaving(true)
    try { await api.delXona(x.id); await reload(); setDelConfirm(null) }
    catch(e){ alert('❌ '+e.message) }
    setSaving(false)
  }
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div>
          ...
          <div style={{fontSize:11,color:'#94a3b8'}}>{(xonalar||[]).length} xona · {bandJami}/{jamiSig} toshak band</div>
        </div>
        <Btn onClick={()=>setModal('add')}>+ Xona qo'shish</Btn>
      </div>

      {/* Stats */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
        {[['Jami xona',(xonalar||[]).length,'#0ea5e9'],['Mavjud',(xonalar||[]).filter(x=>x.band<x.sig).length,'#10b981'],['To\'liq',(xonalar||[]).filter(x=>x.band===x.sig).length,'#ef4444'],[`${bandJami}/${jamiSig} toshak`,null,'#f59e0b']].map(([l,v,c])=>(
          <div key={l} style={card()}><div style={{fontSize:10,fontWeight:700,color:'#94a3b8',textTransform:'uppercase',letterSpacing:.5,marginBottom:5}}>{l}</div><div style={{fontSize:22,fontWeight:800,color:c}}>{v??bandJami+'/'+jamiSig}</div></div>
        ))}
      </div>

      {/* Filter */}
      <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
        {['Barchasi',...TURLAR].map(t=>(
          <button key={t} onClick={()=>setFilterTur(t)} style={{padding:'5px 12px',borderRadius:20,border:'1.5px solid',borderColor:filterTur===t?(TC[t]||'#0ea5e9'):'#e2e8f0',background:filterTur===t?(TC[t]||'#0ea5e9')+'15':'#fff',color:filterTur===t?(TC[t]||'#0ea5e9'):'#64748b',fontSize:12,fontWeight:600,cursor:'pointer'}}>
            {t} {t!=='Barchasi'&&<span style={{opacity:.7}}>({(xonalar||[]).filter(x=>x.tur===t).length})</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? <Loading/> : (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:12}}>
          {fil.map(x=>{
            const pct = x.sig>0 ? Math.round(x.band/x.sig*100) : 0
            ...
            return (
              <div key={x.id} style={card({position:'relative',overflow:'hidden'})}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                  <div>
                    <div style={{fontSize:20,fontWeight:800}}>#{x.raqam}</div>
                    <div style={{fontSize:11,color:col,fontWeight:600}}>{x.tur}</div>
                  </div>
                  <Bdg label={x.band===x.sig?'To\'liq':'Mavjud'} color={x.band===x.sig?'#ef4444':'#10b981'}/>
                </div>
                ...
                <div style={{display:'flex',gap:6,borderTop:'1px solid #f1f5f9',paddingTop:8}}>
                  <Btn sm ghost color='#0ea5e9' style={{flex:1}} onClick={()=>setModal(x)}>✏️ Tahrir</Btn>
                  <Btn sm ghost color='#ef4444' style={{flex:1}} onClick={()=>setDelConfirm(x)}>🗑 O'chirish</Btn>
                </div>
              </div>
            )
          })}
          ...
          {/* Add new card */}
          <div onClick={()=>setModal('add')} style={{...card(),display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:8,cursor:'pointer',border:'2px dashed #e2e8f0',background:'#fafafa',minHeight:160,transition:'all .2s'}}
            ...
            <div style={{fontSize:13,fontWeight:600,color:'#94a3b8'}}>+ Yangi xona</div>
          </div>
        </div>
      )}
      ...
      {/* Delete Confirm */}
      {delConfirm && (
        <Modal title="🗑 Xonani o'chirish" onClose={()=>setDelConfirm(null)}>
          <div style={{padding:'4px 0 16px',fontSize:14,color:'#475569'}}>
            <b>#{delConfirm.raqam}</b> — {delConfirm.tur} xonasini o'chirishni tasdiqlaysizmi?
            {delConfirm.band>0 && <div style={{marginTop:8,padding:'8px 12px',background:'#fef2f2',borderRadius:8,color:'#ef4444',fontSize:13}}>⚠️ Xonada {delConfirm.band} ta bemor bor!</div>}
          </div>
          <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
            <Btn ghost onClick={()=>setDelConfirm(null)}>Bekor</Btn>
            <Btn color='#ef4444' onClick={()=>del(delConfirm)} disabled={saving}>{saving?'...':"🗑 O'chirish"}</Btn>
          </div>
        </Modal>
      )}
    </div>
...
function XonaModal({xona, isNew, turlar, saving, onSave, onClose}){
  const [f, setF] = useState({...xona})
  const s = k => v => setF(p=>({...p,[k]:v}))
  const submit = () => {
    if(!f.raqam.toString().trim()) return alert('Xona raqamini kiriting!')
    if(+f.sig<1) return alert('Toshaklar soni kamida 1 bo\'lsin!')
    if(+f.band<0) return alert('Band toshaklar 0 dan kam bo\'lmasin!')
    if(+f.band>+f.sig) return alert('Band toshaklar jami toshakdan ko\'p bo\'lmasin!')
    onSave(f)
  }
  return (
    <Modal title={isNew ? "🛏 Yangi xona qo'shish" : `✏️ #${xona.raqam} tahrirlash`} onClose={onClose}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
        <Inp label='Xona raqami *' value={f.raqam} onChange={s('raqam')} placeholder='101'/>
        <Inp label='Qavat' value={f.qavat} onChange={s('qavat')} type='number' placeholder='1'/>
        <Inp label='Tur' value={f.tur} onChange={s('tur')} options={turlar}/>
        <Inp label='Jami toshaklar' value={f.sig} onChange={s('sig')} type='number' placeholder='2'/>
        <Inp label='Band toshaklar' value={f.band} onChange={s('band')} type='number' placeholder='0'/>
      </div>
      {/* Preview */}
      <div style={{marginBottom:14,padding:'10px 12px',background:'#f8fafc',borderRadius:10,border:'1px solid #e2e8f0'}}>
        <div style={{fontSize:11,fontWeight:700,color:'#94a3b8',marginBottom:6}}>KO'RINISH</div>
        <div style={{display:'flex',gap:4}}>
          {Array.from({length:Math.min(+f.sig||0,12)}).map((_,i)=>(
            <div key={i} style={{width:28,height:28,borderRadius:4,background:i<(+f.band||0)?'#0ea5e915':'#f1f5f9',border:'1px solid',borderColor:i<(+f.band||0)?'#0ea5e950':'#e2e8f0',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12}}>
              {i<(+f.band||0)?'🛏':''}
            </div>
          ))}
          {+f.sig>12 && <div style={{fontSize:11,color:'#94a3b8',alignSelf:'center'}}>+{+f.sig-12} ta</div>}
        </div>
        <div style={{fontSize:11,color:'#64748b',marginTop:6}}>{+f.band||0}/{+f.sig||0} toshak band · Qavat {f.qavat}</div>
      </div>
      <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
        <Btn ghost onClick={onClose}>Bekor</Btn>
        <Btn onClick={submit} disabled={saving}>{saving?'Saqlanmoqda...':isNew?'+ Qo\'shish':'💾 Saqlash'}</Btn>
      </div>
    </Modal>
  )
}
...
function ObHavo(){
  ...
  const cities=['Toshkent','Samarqand','Namangan','Andijon','Fargona','Buxoro','Nukus','Qarshi','Termiz','Urganch']

  const fetchW=async(c)=>{
    setWLoading(true);setWErr('')
    try{
      ...
      setWeather({
        ...
        humid:cur.humidity,wind:cur.windspeedKmph,
        vis:cur.visibility,uv:cur.uvIndex,
        daily:d.weather.slice(0,5).map(w=>({
          date:w.date,max:w.maxtempC,min:w.mintempC,
          desc:w.hourly[4]?.weatherDesc[0]?.value||''
        }))
      })
    }catch(e){setWErr("Ob-havo yuklanmadi. Internet ulanishini tekshiring.")}
    setWLoading(false)
  }

  useEffect(()=>{fetchW(city)},[city])
  ...
  const dayN=(ds)=>['Yak','Du','Se','Cho','Pay','Ju','Sha'][new Date(ds).getDay()]

  return <div style={{maxWidth:680,margin:'0 auto'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:18}}>
      <div><h2 style={{fontSize:18,fontWeight:800}}>Ob-havo holati</h2><div style={{fontSize:11,color:'#94a3b8'}}>Tibbiy xodimlar uchun kunlik ma'lumot</div></div>
      <select value={city} onChange={e=>setCity(e.target.value)} style={{background:'#fff',border:'1.5px solid #e2e8f0',borderRadius:9,padding:'8px 14px',fontSize:13,outline:'none',cursor:'pointer',fontFamily:'inherit'}}>
        {cities.map(c=><option key={c}>{c}</option>)}
      </select>
    </div>

    {wLoading&&<div style={{...card({padding:60,textAlign:'center'})}}>
      <div style={{fontSize:40,marginBottom:10}}>🌤️</div>
      <div style={{color:'#94a3b8'}}>Yuklanmoqda...</div>
    </div>}

    {wErr&&<div style={{...card({padding:30,textAlign:'center',borderColor:'#fecaca'})}}>
      <div style={{fontSize:28,marginBottom:8}}>⚠️</div>
      <div style={{color:'#ef4444',marginBottom:12}}>{wErr}</div>
      <button onClick={()=>fetchW(city)} style={{background:'#0ea5e9',color:'#fff',border:'none',borderRadius:8,padding:'8px 20px',cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>Qayta urinish</button>
    </div>}

    {weather&&!wLoading&&<>
      <div style={{background:'linear-gradient(135deg,#0f172a,#1e3a5f)',borderRadius:16,padding:28,marginBottom:14,position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',top:-40,right:-40,width:180,height:180,borderRadius:'50%',background:'rgba(14,165,233,.1)'}}/>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div>
            <div style={{fontSize:13,color:'#64748b',marginBottom:4}}>📍 {city}</div>
            <div style={{fontSize:64,fontWeight:900,color:'#fff',lineHeight:1,letterSpacing:-3}}>{weather.temp}°</div>
            <div style={{fontSize:16,color:'#94a3b8',marginTop:6}}>{icon(weather.desc)} {weather.desc}</div>
            <div style={{fontSize:12,color:'#64748b',marginTop:4}}>His qilish: {weather.feels}°C</div>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {[['💧',weather.humid+'%','Namlik'],['💨',weather.wind+' km/h','Shamol'],['👁',weather.vis+' km','Korinish'],['☀️',weather.uv,'UV']].map(([ic,v,l])=>(
              <div key={l} style={{background:'rgba(255,255,255,.07)',borderRadius:8,padding:'7px 14px',textAlign:'right'}}>
                <div style={{fontSize:10,color:'#64748b'}}>{ic} {l}</div>
      ...
      <div style={card({padding:18,marginBottom:12})}>
        <div style={{fontWeight:700,marginBottom:12}}>5 kunlik prognoz</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8}}>
          {weather.daily.map((d,i)=>(
            <div key={i} style={{background:'#f8fafc',borderRadius:10,padding:'12px 6px',textAlign:'center'}}>
              <div style={{fontSize:11,fontWeight:700,color:'#64748b',marginBottom:5}}>{i===0?'Bugun':dayN(d.date)}</div>
              <div style={{fontSize:22,marginBottom:5}}>{icon(d.desc)}</div>
              <div style={{fontSize:13,fontWeight:800,color:'#ef4444'}}>{d.max}°</div>
              <div style={{fontSize:11,color:'#94a3b8'}}>{d.min}°</div>
            </div>
          ))}
        </div>
      </div>

      <div style={card({padding:18,borderColor:'#bae6fd',background:'#f0f9ff'})}>
        <div style={{fontWeight:700,color:'#0369a1',marginBottom:10}}>🏥 Tibbiy maslahat</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          {[
            +weather.temp>30 && ['☀️','Issiq ob-havo',"Ko'proq suv iching, jaziramadan saqlaning"],
            +weather.temp<5 && ['❄️','Sovuq ob-havo',"Nafas yo'llari kasalliklariga e'tibor"],
            +weather.humid>70 && ['💧','Yuqori namlik','Infeksiya tarqalishi xavfi oshadi'],
            +weather.uv>6 && ['🌞','UV yuqori','Teri kasalliklari bor bemorlarga ehtiyot'],
            ['🌡️','Harorat',+weather.temp>25?'Gipertoniklar nazoratda':+weather.temp<10?'Artrit bemorlari ehtiyot':'Qulay harorat'],
            ['💨','Shamol',+weather.wind>30?'Kuchli — nafas olish qiyinlashishi mumkin':"Shamol me'yorida"],
          ].filter(Boolean).map(([ic,title,text])=>(
            <div key={title} style={{padding:'9px 12px',background:'#fff',borderRadius:8,fontSize:12,color:'#475569',lineHeight:1.5}}>
              {ic} <b>{title}:</b> {text}
            </div>
          ))}
        </div>
