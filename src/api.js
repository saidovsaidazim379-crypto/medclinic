...
export const api = {
  ...
  addXona: async (item) => {
    await delay()
    const data = load()
    if(data.xonalar.find(x=>x.raqam===item.raqam)) throw new Error("Bu raqamli xona allaqachon mavjud!")
    const nd = {...item, id:nxtId(data.xonalar), band: item.band||0}
    data.xonalar.push(nd); save(data); return nd
  },
  ...
  delXona: async (id) => {
    await delay()
    const data = load()
    const xona = data.xonalar.find(x=>x.id===+id)
    if(!xona) throw new Error('Topilmadi')
    if(xona.band>0) throw new Error(`Xonada ${xona.band} ta bemor bor! Avval bemorlarni chiqaring.`)
    data.xonalar = data.xonalar.filter(x=>x.id!==+id)
    save(data); return {ok:true}
  },
